...
app = Flask(__name__)  # שיניתי - הסרתי את ההגדרה הכפולה של Flask
...
def add_contact():
    data = request.json
    # הוספתי בדיקה אם כל השדות קיימים
    if 'email' not in data or 'age' not in data or 'fullName' not in data:
